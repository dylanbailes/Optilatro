...
The **economy/shop layer AND the M1 scoring / joker-wiring track are both closed against the reference doc** — see `docs/M2-reference-audit.md` (all M2 P0s, P1s #4–#12, P2 #13/#15 FIXED) and `docs/M1-fidelity-audit.md` §3–§5 (B1 placeholder consumables, B2 dead hooks, B3 Steel/Mime, B4 Lucky, B5 deck counts — all FIXED by the **joker-fidelity program**: 149 jokers canonicalized + behavior-verified against the doc). Remaining before training: **A5 tag weights**, the **B6 misc items**, and **M2 P2 stakes + stickers (#14)** + **Endless (#16)**. Joker-layer structural audit (`tools/audit_jokers_static.py`, CI-wired) is **CLEAN** (0 dupes / 0 dead / 0 stubs / 0 gaps / 0 type mismatches); 150/150 spec rows covered by `tests/test_joker_spec.py` (68 tests). Full suite **964 passed**, ci_gate 4/4. Random-agent baseline re-measured after the fixes: **2/2000 = 0.10% win** (was 0/1000 = 0.00%), ~2,290 steps/s mean.
...
|---|---|---|
| Jokers registered | 168 | `JOKER_REGISTRY` (150 canonical catalogue keys + 18 alias targets; 150/150 spec rows behavior-verified) |
...
**Post-M2-fix baseline (2026-08-06, all M2 P0/P1 + P2 #13/#15 in, `bench/bench_sim.py --games 1000` × 2 batches):**
```
throughput:           2,204–2,384 (~2,290 mean) steps/s (single env, random agent)
random-agent win rate: 2/2000 = 0.10%  (exactly 1 win per batch — first wins ever)
death by ante (9 = won past ante 8):
  ante 1:   ~94.8%    ante 2:   ~4.7%    ante 3:   ~0.25%
  occasional ante 4–5; max ante 9 (a win)
```
The economy/shop fixes (blind rewards, real prices/weights, real pack/voucher availability) moved the random floor from a hard 0.00% to ~0.1% — still essentially a floor for Tier 1 search to beat, but the death distribution shifted slightly deeper (fewer ante-1 deaths, some runs reach ante 4–5).
...
Obs vouchers 27→32 (+5 dims): env_sim **449**, env_v7 **481**, env_mp **485**. Tests: `tests/test_vouchers.py` (28). Full suite: **818 passed**. **Known approximations (FIXED via M2 unless noted):** shop packs were uniformly weighted -> now the real 15-pack rate table (P2 #13); Merchant/Tycoon were 2x/4x multipliers -> now real 9.6/32 weights (P2 #15); pack jokers rolled no editions -> now roll on `edibuf{ante}` with Hone/Glow Up (P1 #9); Standard-Pack cards had no modifiers -> now roll enhancement/edition/seal (P1 #10); Hone/Glow Up now apply to shop AND Buffoon-pack jokers. Remaining: `dc_reroll_ante` (Director’s Cut spent-this-ante) is not in the obs (hidden state the agent infers).
...
5. **Joker audit — Phase 0 done (2026-08-06):** all 144 catalogue entries now resolve to registered effects (**zero dead shop jokers** — was j_supernova + j_oops). Fixed: **Supernova** registered with its real effect (+Mult per run-play-count of the played hand); **Oops! All 6s** registered under its catalogue key (`j_oops`, was buyable-but-dead); **Ring Master** corrected to real "Showman" semantics (duplicates were already the sim default at the time; the real duplicate-suppression rule — and Showman lifting it — landed later with M2 P1 #7, see `docs/M2-reference-audit.md`). **Boss-hook jokers wired in game.py:** Chicot disables ALL boss abilities by presence (`_boss_effects_on()`); Luchador's sell sets a game-level `boss_disabled_override` (survives the sale, consumed when the boss blind resolves); Matador earns +$8 per ability trigger (`_fire_boss_trigger()` — at blind start for static bosses, per hand for Crimson Heart/Cerulean Bell/The Hook). **Glass shatter implemented** (1-in-4 per scored Glass card, doubled by Oops; shattered cards destroyed permanently). Regression: `tests/test_phase0.py` (21 tests). **Note (2026-08-06):** registry resolution is done and **M1 B1 is FIXED** — jokers now create real rewards (random tarot/spectral keys, `("joker"|"card"|"hand_card", …)` tuples via `game._grant_pending`) instead of parking placeholder strings in `consumable_hand` (8-Ball, Seance, Riff-Raff, Cartomancer, Marble, Certificate, Perkeo, Invisible Joker, Diet Cola, DNA, Superposition, Sixth Sense, Hallucination, Vagabond — see `docs/M1-fidelity-audit.md` §3). **B2 is FIXED (2026-08-06, joker-fidelity program):** every previously-dead hook now dispatches from `game.py`/`shop.py` (`on_shop_enter/leave`, `on_reroll`, `on_booster_opened`, `on_init`, `on_planet_used`, `on_tarot_used`, `on_lucky_trigger`, `on_card_added`, `on_card_destroyed`, `on_booster_skipped`). **Joker-fidelity program complete (2026-08-06):** all 149 real jokers canonicalized + behavior-verified against the reference doc — 35 duplicate registrations and 15 dead non-canonical keys removed (Duo-family xMult now on canonical keys, `j_satellite` shadow fixed), Madness/Steel Joker/Hiker/Campfire/Castle/Stuntman/Turtle Bean live-value bugs fixed, held-in-hand scoring (Steel/Mime/Baron/Blackboard/Shoot the Moon/Raised Fist), full-deck counts (Driver's License/Stone/Glass/Steel Joker), Lucky corrected to 1/5 → +20 Mult + 1/15 → +$20 with Oops doubling and Lucky Cat wired. Structural audit (`tools/audit_jokers_static.py`, CI-wired) reports **0 dupes / 0 dead / 0 stubs / 0 gaps / 0 type mismatches**; `tests/test_joker_spec.py` (68 tests) covers all 150 spec rows; full suite **964 passed** + ci_gate 4/4. Remaining: A5 tag weights, B6 misc items, M2 P2 #14/#16 (see `docs/M1-fidelity-audit.md`).
