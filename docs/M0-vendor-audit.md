...
### 4.4 [P1] Deck/stake configuration doesn't exist
Red Deck (+1 discard) and White Stake are not wired as config; `base_discards = STARTING_DISCARDS` is hardcoded to 3. Add a deck/stake config object (even just Red/White) — otherwise the "Red Deck / White Stake" lock-in from the spec is unrepresentable.

### 4.5 [P2] Skip blind is simplified
Flat `+$5` instead of real skip tags (tag effects themselves are absent). Acceptable for v1; must be flagged whenever interpreting early-game behavior.
...
4. **Deck/stake config:** minimal `DeckConfig`/`StakeConfig` (Red Deck / White Stake default).
5. **Joker audit:** walk the 144-catalogue jokers; move unimplemented/TODO jokers behind the allow-list until verified; add tests for the boss-hook jokers (chicot/matador/luchador/ring_master).
6. **Strengthen tests:** replace smoke boss tests with exact-value tests; add a deck-persistence test; keep `bench/bench_sim.py` as the regression gate (any change must not regress ~3,000 steps/s).
