...
1. Early Copier Gating & Protection in agent_v10.py:
...
2. Deck-Aware Hand Specialization in tools/portfolio.py & agent_v10.py:
   - In `tools/portfolio.py` (`portfolio_target_hand`):
     - For `j_family`: only target "Four of a Kind" if the full deck has >= 6 cards of the same rank (e.g. `max(multiset.values()) >= 6`); otherwise retain high-frequency hand (`main_hand_type` or "Two Pair" / "Pair" / "Flush").
...
4. Run Verification Suite:
   ...
   - Validate Seed 298 and Seed 80 behavior.

Write your report to D:/Optilatro/.agents/teamwork_preview_worker_remedy_m2_gen6/handoff.md and report back.
